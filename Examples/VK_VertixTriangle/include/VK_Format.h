...
struct formatInfo {
    enum rawDataType :uint8_t {
        other,        //0，No data or different channels
        integer,      //1，The data type is an integer
        floatingPoint //1，The data type is a floating point number
    };
    uint8_t componentCount;   //通道数
    uint8_t sizePerComponent; //The size of each channel, 0 means compression, or unequal, or less than 1
    uint8_t sizePerPixel;     //The size of each pixel, 0 means compression
    uint8_t rawDataType;      //The underlying data type
};
